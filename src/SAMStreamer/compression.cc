...
int compress_line(Arithmetic_stream as, sam_block samBlock, FILE *funmapped, uint8_t lossiness, int calq, Arithmetic_stream as1, uint64_t context[25][6], char* prefix, std::vector<calq::SAMRecord> &samRecords)  {
    try {
         /*if (calq) {
            //Max33 Phred+33 [0,93]
...
void* compress(void *thread_info){
    ...
    while (compress_line(as, samBlock, info.funmapped, info.lossiness, info.calqmode, as1, context, prefix, samRecords)) {
        ++lineCtr;
        if (lineCtr % 10000 == 0) {
